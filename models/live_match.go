...
type LiveMatch struct {
	...
	MatchID                    nsproto.MatchID       `gorm:"column:match_id;unique_index;not null"`
	ServerSteamID              steamid.SteamId       `gorm:"column:server_steam_id;not null"`
	LobbyID                    nsproto.LobbyID       `gorm:"column:lobby_id;unique_index;not null"`
	LobbyType                  nsproto.LobbyType     `gorm:"column:lobby_type"`
	LeagueID                   nsproto.LeagueID      `gorm:"column:league_id"`
	SeriesID                   nsproto.SeriesID      `gorm:"column:series_id"`
	GameMode                   nsproto.GameMode      `gorm:"column:game_mode"`
	...
	BuildingState              nsproto.BuildingState `gorm:"column:building_state"`
...
func LiveMatchDotaProto(pb *protocol.CSourceTVGameSmall) *LiveMatch {
	return &LiveMatch{
		...
		LeagueID:                   nsproto.LeagueID(pb.GetLeagueId()),
		SeriesID:                   nsproto.SeriesID(pb.GetSeriesId()),
		LobbyID:                    pb.GetLobbyId(),
		LobbyType:                  nsproto.LobbyType(pb.GetLobbyType()),
		GameMode:                   nsproto.GameMode(pb.GetGameMode()),
		...
		BuildingState:              nsproto.BuildingState(pb.GetBuildingState()),
