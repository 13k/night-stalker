...
var LiveMatchStatsModel = (*LiveMatchStats)(nil)
...
type LiveMatchStats struct {
	...
	MatchID                    nsproto.MatchID      `gorm:"column:match_id;index;not null"`
	ServerSteamID              steamid.SteamId      `gorm:"column:server_steam_id;index;not null"`
	LeagueID                   nsproto.LeagueID     `gorm:"column:league_id"`
	LeagueNodeID               nsproto.LeagueNodeID `gorm:"column:league_node_id"`
	...
	GameState                  nsproto.GameState    `gorm:"column:game_state"`
...
func LiveMatchStatsDotaProto(pb *protocol.CMsgDOTARealtimeGameStatsTerse) *LiveMatchStats {
	return &LiveMatchStats{
		...
		LeagueID:                   nsproto.LeagueID(pb.GetMatch().GetLeagueId()),
		LeagueNodeID:               pb.GetMatch().GetLeagueNodeId(),
		GameTimestamp:              pb.GetMatch().GetTimestamp(),
		GameTime:                   pb.GetMatch().GetGameTime(),
		GameMode:                   nsproto.GameMode(pb.GetMatch().GetGameMode()),
		GameState:                  nsproto.GameState(pb.GetMatch().GetGameState()),
